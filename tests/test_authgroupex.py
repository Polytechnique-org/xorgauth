...
class AuthGroupeXTests(TestCase):
    ...
    def _get_logout_req_url(privkey, return_url, session=None):
        ...
        challenge = hashlib.sha1(b"".join(struct.pack(b"B", random.randrange(0, 256)) for i in range(64)))
    ...
    def _check_resp_auth(self, privkey, return_url, challenge, location, additional_fields=None):
        ...
        self.assertTrue(location.startswith(return_url + "?"), "unexpected Location header: %r" % location)
        ...
        self.assertEqual(known_resp_fields, set(query_params.keys()), "unexpected fields in response")
    ...
    def test_unlogged_request(self):
        ...
        self.assertTrue(
            resp["Location"].startswith(login_url + "?"), "unexpected Location header: %r" % resp["Location"]
        )
    ...
    def test_logged_request__groupadm(self):
        ...
        c = Client()
        self.assertTrue(c.login(username="louis.vaneau.1829", password="Depuis Vaneau!"))
        requrl, challenge = self._get_req_url(self.client_simple.privkey, "https://example.com/", group="X1829")
        resp = c.get(requrl)
        self.assertEqual(302, resp.status_code)
        query_params, expected_auth = self._check_resp_auth(
            self.client_simple.privkey, "https://example.com/", challenge, resp["Location"]
    ...
    def test_logout_phpsession_request(self):
        ...
        requrl, _ = self._get_logout_req_url(
            self.client_simple.privkey, "https://example.com/logout", session="0123456789"
        )
        resp = c.get(requrl)
        self.assertEqual(302, resp.status_code)
